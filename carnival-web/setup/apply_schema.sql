...
CREATE TABLE IF NOT EXISTS password_reset_tokens (
        ...
        user_id INTEGER UNIQUE NOT NULL,
        -- foreign key to users table 1:1
        token VARCHAR(36) UNIQUE NOT NULL,
        -- can technically be char cause UUID length is fixed 
        created_at INTEGER DEFAULT (strftime('%s', 'now')),
        -- DATETIME exists but something something SQLx compatibility
        expires_at INTEGER NOT NULL,
        -- maybe we can make it compatible... but I cba
...
create table if not exists brackets_thru(
        id integer primary key,
        user_id integer not null,
        bracket_id integer not null
);
create table if not exists bracket_keys (
